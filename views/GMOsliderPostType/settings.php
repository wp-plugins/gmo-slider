<table width="100%">
	<?php $groups = array(); ?>
	<?php if(count($settings) > 0): ?>
	<?php foreach($settings as $key => $value): ?>

	<?php if( !isset($value, $value['type'], $value['default'], $value['description']) || !is_array($value)) continue; ?>

	<?php if(!empty($value['group']) && !isset($groups[$value['group']])): $groups[$value['group']] = true; ?>
	<tr>
		<td colspan="2" style="border-bottom: 1px solid #e5e5e5; text-align: center;">
			<span style="display: inline-block; position: relative; top: 14px; padding: 0 12px; background: #fff;">
				<?php echo $value['group']; ?> <?php _e('settings', 'gmoslider'); ?>
			</span>
		</td>
	</tr>
	<tr>
		<td colspan="2"></td>
	</tr>
	<?php endif; ?>
	<tr>
		<td><?php echo $value['description']; ?></td>
		<td><?php echo GMOsliderSettingsHandler::getInputField(GMOsliderSettingsHandler::$settingsKey, htmlspecialchars($key), $value); ?></td>
	</tr>

	<?php endforeach; ?>
	<?php endif; ?>
</table>